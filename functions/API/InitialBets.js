  ...
  try {
    ...
    if (Date.now() > 1781204400000) {
      if (Date.now() < 1782673200000) {
        const checkIfChanged = await db.prepare("SELECT betChanged FROM UserScores WHERE ID = ?").bind(userID).first();
        ...
        if (!checkIfChanged || checkIfChanged.betChanged == 0) {
          await db.prepare("UPDATE UserScores SET betChanged = 1, points = points - 5, championBet = ? WHERE ID = ?")
            .bind(reqBody.championBet, userID).run();
        ...
        await db.prepare("UPDATE UserScores SET championBet = ? WHERE ID = ?").bind(reqBody.championBet, userID).run();
    ...
    await db.prepare("UPDATE UserScores SET championBet = ?, topScorerBet = ? WHERE ID = ?")
      .bind(reqBody.championBet, reqBody.topScorerBet, userID).run();
